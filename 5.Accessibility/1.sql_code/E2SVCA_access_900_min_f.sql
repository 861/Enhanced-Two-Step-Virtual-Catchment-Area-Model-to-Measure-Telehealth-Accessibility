CREATE table glassy-automata-358521.access.od_pop as SELECT *, pop*min_f_w ppotent from (select * from `glassy-automata-358521.access.od_900_min_f` od_900_min_f join `glassy-automata-358521.access.pop` pop on pop.popid = od_900_min_f.orig);
...
CREATE table glassy-automata-358521.access.access_900_min_f as SELECT orig, sum(R) acc900 from `glassy-automata-358521.access.ratio` GROUP BY orig;
